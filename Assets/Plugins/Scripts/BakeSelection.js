 ...
 public  function BakeSelectedObjects(objs:System.Object[],caller:System.Object)
    ...
    var initial:LightmapData[] =  new LightmapData[LightmapSettings.lightmaps.Length];
				...
				for(var i:int =0;i<LightmapSettings.lightmaps.Length;i++)
				{
					...
					initial[i].lightmapFar = LightmapSettings.lightmaps[i].lightmapFar;
					initial[i].lightmapNear = LightmapSettings.lightmaps[i].lightmapNear;
			}
		...
		var originalSelectedAtals:Vector4[] = new Vector4[selectedRenderers.Count];
				...
				{
					originalScales[i] = renderers[i].lightmapTilingOffset;
       ...
       var check:boolean = method2.Invoke(param.caller,objs);
         while (check)
         {
			...
			originalSelectedAtals[x] = mr.lightmapTilingOffset;
			}
       ...
       method5.Invoke(param.caller,null);
        ...
        for(i=0;i<cachedObjects.Count;i++)
        {
          ...
          for(i=0;i<renderers.Length;i++)
			...
			mr.lightmapTilingOffset = originalSelectedAtals[x];
			}  
				...
				{
				Debug.Log(initial[i].lightmapFar);
				...
				copiedData[x].lightmapFar =LightmapSettings.lightmaps[x].lightmapFar;
				copiedData[x].lightmapNear =LightmapSettings.lightmaps[x].lightmapNear;
				}
				if(initial[i].lightmapFar!=null)
				copiedData[i].lightmapFar = initial[i].lightmapFar;
				if(initial[i].lightmapNear!=null)
				copiedData[i].lightmapNear = initial[i].lightmapNear;
