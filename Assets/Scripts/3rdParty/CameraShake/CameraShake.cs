...
public class CameraShake : MonoBehaviour
{
	...
	public void DoShake()
	{
		...
		{
			StartCoroutine(DoShake_Internal(cam, seed, this.numberOfShakes, this.shakeAmount, this.rotationAmount, this.distance, this.speed, this.decay, this.multiplyByTimeScale));
		}
	}
	public void DoShake(int numberOfShakes, Vector3 shakeAmount, Vector3 rotationAmount, float distance, float speed, float decay, bool multiplyByTimeScale = true)
	{
		Vector3 seed = Random.insideUnitSphere;
		
		foreach(Camera cam in cameras)
		{
			StartCoroutine(DoShake_Internal(cam, seed, numberOfShakes, shakeAmount, rotationAmount, distance, speed, decay, multiplyByTimeScale));
		}
	}
	...
	private IEnumerator DoShake_Internal(Camera cam, Vector3 seed, int numberOfShakes, Vector3 shakeAmount, Vector3 rotationAmount, float distance, float speed, float decay, bool multiplyByTimeScale)
	{
